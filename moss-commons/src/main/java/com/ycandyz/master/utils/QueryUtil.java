...
public class QueryUtil {
    ...
    // 构建QueryWrapper
    public static <T> QueryWrapper buildWrapper(T query) {
        return buildWrapper(query, true);
    }

    // 构建QueryWrapper
    public static <T> QueryWrapper buildWrapper(T query, boolean introspect) {
        ...
        for (Field field : ReflectUtil.getAllFields(query.getClass())) {
            ...
            switch (annotation.condition()) {
                ...
                case FIND_IN_SET:
                	if (value != null) {
                		queryWrapper.apply(" FIND_IN_SET ("+key+",'"+value+"')  ");
                    }
                    break;
                case MATCH:
                	if (value != null) {
                		queryWrapper.apply(" MATCH ("+key+") AGAINST ('"+value+"' IN BOOLEAN MODE)");
                    }
                    break;
                default:
                    break;
