...
public class TabooCheckServiceImpl implements TabooCheckService {
    ...
    public List<String> check(String txt) {
        List list = TabooCheck.check(allTaboosLists, txt);
        return list;
    }

    @PostConstruct
    public void init(){
        List<String> allTaboosList = new ArrayList<>();
        ...
        allTaboosList.stream().forEach(s->allTaboosLists.addAll(MyCollectionUtils.parseIds(s)));
    }
}
