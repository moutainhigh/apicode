...
import com.ycandyz.master.dao.mall.goodsManage.GoodsMallSkuDao;
import com.ycandyz.master.dao.mall.goodsManage.GoodsMallSkuSpecDao;
...
import com.ycandyz.master.service.mall.goodsManage.GoodsMallItemService;
...
@Service
@Slf4j
public class GoodsMallItemServiceImpl implements GoodsMallItemService {
    ...
    @Resource
    private GoodsMallSkuDao goodsMallSkuDao;

    @Resource
    private GoodsMallSkuSpecDao goodsMallSkuSpecDao;
    ...
    private void insertMallSku(String itemNo, MallSkuVO[] skus) {
        ...
        if (skus != null && skus.length > 0){
            ...
            for (MallSkuVO mvo : skus) {
                ...
                goodsMallSkuDao.addMallSku(mallSku);
                ...
                if (skuSpecs != null && skuSpecs.length > 0){
                    for (MallSkuSpecsVO mspec: skuSpecs) {
                        ...
                        goodsMallSkuSpecDao.addMallSkuSpec(mallSkuSpec);
                    }
                }
            i++;
            }
    ...
    */
    @Override
    public MallItemDTO select(String itemNo) {
        UserVO userVO = UserRequest.getCurrentUser();
       log.info("查询商品详情入参userVO:{};itemNo:{}",userVO,itemNo);
        ...
        List<MallSku> mallSkus = goodsMallSkuDao.selMallSkuByitemNo(itemNo);
        if (mallSkus != null && mallSkus.size() > 0){
            for (MallSku m: mallSkus) {
                List<MallSkuSpec> mallSkuSpecs = goodsMallSkuSpecDao.selMallSkuSpecBySkuNo(m.getSkuNo());
        ...
        MallCategoryDTO mallCategoryDTO = mallCategoryService.select(mallItem.getCategoryNo());
    ...
    */
    @Override
    public int opr(MallItemOprVO mallItemOprVO) {
        UserVO userVO = UserRequest.getCurrentUser();
    ...
    */
    ...
    public void update(MallItemVO mallItemVO) {
        ...
        if (skus != null && skus.length > 0){
            int i2 = goodsMallSkuDao.deleteSkuDao(itemNo);
            insertMallSku(itemNo,skus);
        }
