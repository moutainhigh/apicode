...
public class MallItemVideoController extends BaseController<MallItemVideoServiceImpl,MallItemVideo,MallItemVideoQuery> {
    ...
    @PostMapping
    public CommonResult<MallItemVideo> insert(@Validated(ValidatorContract.OnCreate.class) MallItemVideo entity, @ApiParam(name="videoFile",value="视频文件") @RequestParam(required = false) MultipartFile videoFile, @ApiParam(name="imgFile",value="缩略图") MultipartFile imgFile) {
        return result(service.insert(entity,videoFile,imgFile),entity,"上传失败!");
    }
	
	@ApiOperation(value = "通过视频ID更新视频/缩略图")
    @PostMapping(value = "{id}")
	public CommonResult<MallItemVideo> updateById(@PathVariable Long id,@Validated(ValidatorContract.OnUpdate.class) MallItemVideo entity, @ApiParam(name="videoFile",value="视频文件") MultipartFile videoFile, @ApiParam(name="imgFile",value="缩略图") MultipartFile imgFile) {
        entity.setId(id);
